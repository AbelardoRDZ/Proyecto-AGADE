...
function RegistroFutbol({ auth, registroFutbol }) {
    ...
    const submit = (e) => {
        e.preventDefault()
        //console.log(data);
        post(route('Unirfutbol.store'), { onSuccess: () => reset() })
    }

    return (
        <AuthenticatedLayout auth={auth}>
            <Head title='Unirse Futbol' />
            <div className='max-w-2x1 mx-auto p-4 sm:p-6 lg:p-8'>
                ...
                <form onSubmit={submit}>
                    ...
                    >
                        {registroFutbol.map(futbol => (
                            <option selected key={futbol.id}>{futbol.nombre}</option>
                        ))}
                    </select><br /><br />
                    ...
                    >
                        {registroFutbol.map(futbol => (
                            <option selected key={futbol.id}>{futbol.creador}</option>
                        ))}
                    </select><br /><br />
                ...
                <div className='mt-6 bg-indigo-400 rounded-lg divide-y-4 shadow-lg'>
                    {
                        registroFutbol.map(futbol =>
                            <Futbol key={futbol.id} futbol={futbol} />

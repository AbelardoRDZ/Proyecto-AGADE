...
function RegistroFutbol({ auth, registrovoly }) {
    ...
    const submit = (e) => {
        e.preventDefault()
        //console.log(data);
        post(route('Unirvoly.store'), { onSuccess: () => reset() })
    }

    return (
        <AuthenticatedLayout auth={auth}>
            <Head title='Unirse Voly' />
            <div className='max-w-2x1 mx-auto p-4 sm:p-6 lg:p-8'>
                ...
                <div className='mt-6 bg-indigo-400 rounded-lg divide-y-4 shadow-lg'>
                    {
                        registrovoly.map(voly =>
                            <Voly key={voly.id} voly={voly} />
